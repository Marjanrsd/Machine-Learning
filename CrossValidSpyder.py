# -*- coding: utf-8 -*-
"""
Created on Fri Aug  9 14:30:26 2024
...
def train_worker(epochs, model, loss_func, optimizer, evaluate_model, train, test, q):
    print(123) 
    import torch
    ...
    q.put(accuracy)
    # print(f'Worker {split} finished with accuracy: {accuracy}')
...
if __name__ == '__main__':
   ...
   # import torch
    ...
    for k in num_rounds:
        new_x_tensor = x_tensor.detach().clone().to(device)
        new_xtest_tensor = xtest_tensor.detach().clone().to(device)
        ROI_names = np.array(all_ROI_names.copy())
    
        for l in range(k):
            file_path = f'./{l+6}_subset_sampling_acc.npy'
        ...
        q = m.Queue()
        train_pool = mp.Pool(3)  # Adjust number of processes for split level parallelism
        ...
        for i in range(num_ROIs):
            ...
            q = m.Queue()
            train_pool = mp.Pool(3) #reinitialize pool
            for split in range(num_splits):
                X, y = x_tensor.detach().clone().to(device), y_tensor.detach().clone().to(device)
                ...
                model = ChurnModel(n_input_dim).to(device)
            ...
            split_accuracies = []
            while not q.empty():
                split_accuracies.append(q.get())
                print("I'm in while not q.empty loop")
        
            avg_acc = np.mean(split_accuracies)
            accuracies.append(avg_acc)
            print("we saved accuracies for 2 splits! ready for sending new processes")
        train_pool.close()
        train_pool.join()
    
        # while not q.empty():
        #     i, avg_accuracy = q.get()
        #     accuracies.append(avg_accuracy)
        #     print(f'{len(accuracies)} models trained! c:')
        #     print("ACCURACY", avg_accuracy)
        
        save_file = f'./{k+6}_subset_sampling_acc.npy'
