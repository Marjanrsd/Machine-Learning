...
def train_worker(epochs, model, loss_func, optimizer, evaluate_model, train, test, q, cuda_device):
    ...
    q.put((accuracy, cuda_device))
    


# def manage_processes(epochs, model, loss_func, optimizer, evaluate_model, train, test):
#     #q = mp.Queue()
#     num_processes = 9
#     gpu_limits = {0: 6, 1: 3}  # 6 processes on CUDA:0, 3 processes on CUDA:1
#     gpu_usage = {0: 0, 1: 0}
#     processes = []

#     with mp.Pool(num_processes) as pool:
#         while True:
#             # Start new processes if there is capacity
#             for gpu_device in [0] * gpu_limits[0] + [1] * gpu_limits[1]:
#                 if gpu_usage[gpu_device] < gpu_limits[gpu_device]:
#                     args = (epochs, model, loss_func, optimizer, evaluate_model, train, test, q, gpu_device)
#                     result = pool.apply_async(func=train_worker, args=args)
#                     processes.append((result, gpu_device))
#                     gpu_usage[gpu_device] += 1

#             # Check for completed processes and replace them
#             while processes:
#                 time.sleep(2)
                
#                 # Check if any process has finished
#                 for result, gpu_device in processes[:]:
#                     if result.ready():
#                         accuracy, cuda_device = result.get()
#                         print(f"Process on CUDA:{cuda_device} finished with accuracy: {accuracy}")

#                         # Remove the completed process from the list
#                         processes.remove((result, gpu_device))
#                         gpu_usage[gpu_device] -= 1

#                         # Start new process for the GPU if there is capacity
#                         if gpu_usage[gpu_device] < gpu_limits[gpu_device]:
#                             args = (epochs, model, loss_func, optimizer, evaluate_model, train, test, q, gpu_device)
#                             new_result = pool.apply_async(func=train_worker, args=args)
#                             processes.append((new_result, gpu_device))
#                             gpu_usage[gpu_device] += 1
...
if __name__ == '__main__':
    ...
    #sample 60 trials from each condition
    ...
    #!pip install multiprocessing # only have to run on new PCs. Documentation at this point.
    ...
    print ("rob jooni")
    #def train_region_worker(i, new_x_tensor, y_tensor, bs, epochs, num_splits):
   ...
   # import torch
    ...
    for k in num_rounds:
        ...
        q = m.Queue()
        #train_pool = mp.Pool(15)  # Adjust number of processes for split level parallelism
        ...
        for i in range(num_ROIs):
            ...
            for split in range(num_splits):
                #X, y = x_tensor.detach().clone().to(f"cuda:{cuda_device}"), y_tensor.detach().clone().to(f"cuda:{cuda_device}")
                ...
                # model = ChurnModel(n_input_dim).to(f"cuda:{cuda_device}")
                # optimizer = torch.optim.Adam(model.parameters(), lr=0.0003)
                # loss_func = torch.nn.BCELoss()
                #args = (epochs, model, loss_func, optimizer, evaluate_model, train_dl_split, test_loader_split, q)
                #train_pool.apply_async(func=train_worker, args=args, error_callback=ecb)
                #manage_processes(50, model, loss_func, optimizer, evaluate_model, train_dl_split, test_loader_split)
                num_processes = 9
                gpu_limits = {0: 6, 1: 3}  # 6 processes on CUDA:0, 3 processes on CUDA:1
                gpu_usage = {0: 0, 1: 0}
                processes = []
                ...
                while True:
                   for gpu_device in [0] * gpu_limits[0] + [1] * gpu_limits[1]:
                       if gpu_usage[gpu_device] < gpu_limits[gpu_device]:
                           ...
                           optimizer = torch.optim.Adam(model.parameters(), lr=0.0003)
                           loss_func = torch.nn.BCELoss()
                           args = (epochs, model, loss_func, optimizer, evaluate_model, train_dl_split, test_loader_split, q, gpu_device)
                           process = mp.Process(target=train_worker, args=args)
                           process.start()
                           processes.append((process, gpu_device))
                           gpu_usage[gpu_device] += 1
                           break  # break the while True loop and continue with next split
    
                   # Manage process completion
                   for proc, gpu_device in processes[:]:
                       if not proc.is_alive():
                           proc.join()
                           processes.remove((proc, gpu_device))
                           gpu_usage[gpu_device] -= 1
            #train_pool.close()
            #train_pool.join()
            #del train_pool
    ...
    data = np.load(r"./7_subset_sampling_acc.npy")
    print(data)
